...
const handleKeyUp = (e, store) => {
  e.stopPropagation();
  store.handleKeyUp(e.nativeEvent);
  e.preventDefault();
};

const handleClick = (contentElementRef) => {
  setTimeout(() => {
    if (!contentElementRef) return;
    if (
      !document.activeElement ||
      !contentElementRef.contains(document.activeElement)
    ) {
      contentElementRef.focus();
    }
  });
};
...
const renderMainMenu = (instruments, features) => {
  return (
    <ScrollView>
      <Observer>
        {() => (
...
const SecondaryContent = ({ registerWheelListener, store, features }) => {
  return (
    <>
      <BottomArea>
        <WheelController
      ...
      <section className={tw`absolute bottom-0 inset-x-0 h-[24px] flex bg-#252423 border-t border-#454443`}>
        <StatusBar />
      </section>
      <Observer>
        {() => (
...
export const MainView = ({ features, store }) => {
  ...
  return (
    <div>
      ...
      <MainContent
        tabIndex={0}
        onClick={() => handleClick(contentElementRef.current)}
        onKeyDown={(e) => handleKeyDown(e, store)}
        onKeyUp={(e) => handleKeyUp(e, store)}
      ...
      >
        <Switch>
          {instruments.map((instrument) => {
            ...
            />,
            <Route key="@@main" render={() => renderMainMenu(instruments, features)} />,
          ])}
        </Switch>
      </MainContent>
      <SecondaryContent
